...
public static class DbSeeder
{
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        // Docker SQL Server can become healthy slightly after API startup.
        // Retry loop prevents repeated crash-restart cycles on first boot.
        const int maxAttempts = 10;
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            try
            {
                // GetMigrations() is synchronous and lives directly on DatabaseFacade —
                // no extension-method resolution issues across any EF version.
        ...
        await ParentSeeder.SeedAsync(userManager);           // was missing
        ...
        // ── Relations ─────────────────────────────────────────────────────────
        await ParentStudentLinkSeeder.SeedAsync(db, userManager); // was missing
