...
public class LmsPlatformService : ILmsPlatformService
{
    ...
    public async Task<LiveSessionStatusDto> GoLiveAsync(string userId, bool isAdmin, int sessionId, CancellationToken ct = default)
    {
        var session = await _db.LiveSessions.Include(l => l.Course)
            .FirstOrDefaultAsync(l => l.Id == sessionId, ct)
            ?? throw new NotFoundException(nameof(LiveSession), sessionId);
        if (!isAdmin && session.InstructorId != userId)
            throw new ForbiddenException("Only the session instructor or an admin can start a live session.");
        session.GoLive();
        await _db.SaveChangesAsync(ct);
        return new LiveSessionStatusDto(session.Id, session.Title, session.Status.ToString());
    }

    public async Task<LiveSessionStatusDto> CompleteSessionAsync(string userId, bool isAdmin, int sessionId, CancellationToken ct = default)
    {
        var session = await _db.LiveSessions
            ...
            ?? throw new NotFoundException(nameof(LiveSession), sessionId);
        if (!isAdmin && session.InstructorId != userId)
            throw new ForbiddenException("Only the session instructor or an admin can complete this session.");
        session.Complete();
        await _db.SaveChangesAsync(ct);
        return new LiveSessionStatusDto(session.Id, session.Title, session.Status.ToString());
    }

    public async Task CancelSessionAsync(string userId, bool isAdmin, int sessionId, CancellationToken ct = default)
    {
        var session = await _db.LiveSessions.Include(l => l.Course)
            .FirstOrDefaultAsync(l => l.Id == sessionId, ct)
            ?? throw new NotFoundException(nameof(LiveSession), sessionId);
        if (!isAdmin && session.InstructorId != userId)
            throw new ForbiddenException("Only the session instructor or an admin can cancel this session.");
        session.Cancel();
        await _db.SaveChangesAsync(ct);
    }
    ...
    public async Task<AttendanceDto> CheckInAsync(string studentId, int sessionId, CancellationToken ct = default)
    {
        var session = await _db.LiveSessions.FindAsync([sessionId], ct)
            ?? throw new NotFoundException(nameof(LiveSession), sessionId);
        if (session.Status != LiveSessionStatus.Live)
            throw new InvalidOperationException("Check-in is only allowed when the session is live.");
        var enrolled = await _db.Enrollments.AnyAsync(e => e.StudentId == studentId && e.CourseId == session.CourseId, ct);
        if (!enrolled) throw new ForbiddenException("You must be enrolled in the course to check in.");
        var existing = await _db.SessionAttendances
            .FirstOrDefaultAsync(a => a.LiveSessionId == sessionId && a.StudentId == studentId, ct);
        if (existing != null) return await MapAttendanceDtoAsync(existing, ct);
        var now = DateTime.UtcNow;
        var attendance = new SessionAttendance
        {
            LiveSessionId = sessionId,
            StudentId     = studentId,
            Status        = (now - session.StartTime).TotalMinutes > LateGraceMinutes ? AttendanceStatus.Late : AttendanceStatus.Present,
            JoinedAt      = now
        };
        _db.SessionAttendances.Add(attendance);
        await _db.SaveChangesAsync(ct);
        return await MapAttendanceDtoAsync(attendance, ct);
    }

    public async Task<AttendanceDto> CheckOutAsync(string studentId, int sessionId, CancellationToken ct = default)
    {
        var attendance = await _db.SessionAttendances
            .FirstOrDefaultAsync(a => a.LiveSessionId == sessionId && a.StudentId == studentId, ct)
            ?? throw new NotFoundException("Attendance record not found. Did you check in?");
        if (attendance.LeftAt.HasValue) return await MapAttendanceDtoAsync(attendance, ct);
        attendance.LeftAt = DateTime.UtcNow;
        if (attendance.JoinedAt.HasValue)
            attendance.DurationMinutes = (int)(attendance.LeftAt.Value - attendance.JoinedAt.Value).TotalMinutes;
        await _db.SaveChangesAsync(ct);
        return await MapAttendanceDtoAsync(attendance, ct);
    }

    public async Task<SessionAttendanceSummaryDto> GetAttendanceSummaryAsync(string userId, bool isAdmin, int sessionId, CancellationToken ct = default)
    {
        var session = await _db.LiveSessions
            .Include(l => l.Attendances)
            .Include(l => l.Course)
            ...
            ?? throw new NotFoundException(nameof(LiveSession), sessionId);
        if (!isAdmin && session.InstructorId != userId)
            throw new ForbiddenException("Only the instructor or admin can view the attendance roster.");
        var userIds = session.Attendances.Select(a => a.StudentId).ToList();
        var users = await _db.Users
            .Where(u => userIds.Contains(u.Id))
            .Select(u => new { u.Id, Name = u.UserName ?? u.Email ?? u.Id })
            .ToDictionaryAsync(u => u.Id, u => u.Name, ct);
        var dtos = session.Attendances
            .OrderBy(a => a.JoinedAt ?? DateTime.MaxValue)
            .Select(a => MapAttendanceDto(a, users.GetValueOrDefault(a.StudentId, a.StudentId)))
            .ToList();
        return new SessionAttendanceSummaryDto(
            session.Id, session.Title, dtos.Count,
            dtos.Count(d => d.Status is "Present" or "Late"),
            dtos.Count(d => d.Status == "Absent"),
            dtos.Count(d => d.Status == "Late"),
            dtos);
    }

    public async Task<AttendanceDto> UpdateAttendanceAsync(string userId, bool isAdmin, int attendanceId, UpdateAttendanceRequest request, CancellationToken ct = default)
    {
        var attendance = await _db.SessionAttendances.Include(a => a.LiveSession)
            .FirstOrDefaultAsync(a => a.Id == attendanceId, ct)
            ?? throw new NotFoundException(nameof(SessionAttendance), attendanceId);
        if (!isAdmin && attendance.LiveSession?.InstructorId != userId) throw new ForbiddenException();
        if (!Enum.TryParse<AttendanceStatus>(request.Status, ignoreCase: true, out var status))
            throw new ArgumentException($"Invalid attendance status '{request.Status}'.");
    ...
    public async Task<IReadOnlyList<AttendanceDto>> GetMyAttendanceAsync(string studentId, CancellationToken ct = default) =>
        await _db.SessionAttendances
            .Where(a => a.StudentId == studentId)
            .OrderByDescending(a => a.JoinedAt)
            .Select(a => new AttendanceDto(a.Id, a.LiveSessionId, a.StudentId, studentId,
                a.Status.ToString(), a.JoinedAt, a.LeftAt, a.DurationMinutes, a.Note))
            .ToListAsync(ct);

    private async Task<AttendanceDto> MapAttendanceDtoAsync(SessionAttendance a, CancellationToken ct)
    {
        var user = await _db.Users.FindAsync([a.StudentId], ct);
        return MapAttendanceDto(a, user?.UserName ?? user?.Email ?? a.StudentId);
    }

    private static AttendanceDto MapAttendanceDto(SessionAttendance a, string studentName) =>
        new(a.Id, a.LiveSessionId, a.StudentId, studentName,
            a.Status.ToString(), a.JoinedAt, a.LeftAt, a.DurationMinutes, a.Note);
