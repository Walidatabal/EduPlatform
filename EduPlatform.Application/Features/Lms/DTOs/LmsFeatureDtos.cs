...
public record LiveSessionStatusDto(int Id, string Title, string Status);

public record AttendanceDto(int Id, int LiveSessionId, string StudentId, string StudentName, string Status, DateTime? JoinedAt, DateTime? LeftAt, int? DurationMinutes, string? Note);
public record SessionAttendanceSummaryDto(int LiveSessionId, string SessionTitle, int TotalRegistered, int PresentCount, int AbsentCount, int LateCount, IReadOnlyList<AttendanceDto> Attendees);
public record UpdateAttendanceRequest(string Status, string? Note);
